...
const OSearch = ({ intl: { formatMessage } }) => {
  ...
  const history = useHistory();
   ...
   const handleClick = e => {
    ...
    // const apiUrl = `https://ticker-2e1ica8b9.now.sh//keyword/`+e;
    // fetch(apiUrl)
    // .then((res) => res.json())

    const apiUrl = 'https://ticker-2e1ica8b9.now.sh/keyword/adient'
    fetch(apiUrl)
    .then((response) => response.json())
    .then((data) => localStorage.setItem('stockname', data[0].symbol), console.log(data + "us"))
    // .then((data) => console.log('Our data', data))
    // .then((data) => {
    //   localStorage.setItem('stockname', data[0].symbol);
    //   console.log(data + "As Symbol")
    // })
      
      // .then((repos) => {
      //   setAppState({ loading: false, stockName:repos.symbol, repos: repos.exchange, name:repos.companyName, sector:repos.sector, description:repos.description });
     
      //   localStorage.setItem('stockname', repos.symbol);
