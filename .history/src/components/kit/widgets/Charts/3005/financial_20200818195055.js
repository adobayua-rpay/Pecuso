...
export default function RecipeReviewCard() {
  ...
  useEffect(() => {
    setAppStates({ loading: true });
    const apiUrl = `https://sandbox.iexapis.com/stable/stock/aapl/financials?token=Tsk_b918148a96ad4a198cf793bbbccac54d`;
    fetch(apiUrl)
      .then((res) => res.json())
      .then((reposo) => {
        setAppStates({ loading: false, grossProfit: reposo.grossProfit, costofrevenue:reposo.costOfRevenue});
      });
  }, [setAppStates]);
...
// class App extends React.Component {
  ...
  <Collapse defaultActiveKey={['1']} onChange={callback}>
    <Panel header="Report Date" key="1">
      <p>{text}</p>
      <strong>{appStates.costofrevenue}</strong>
    ...
    <Panel header="Cost of Revenue" key="3">
      <p>{costofrevenue}</p>
      <strong>{appStates.grossProfit}</strong>
