...
let comparison = '';
  ...
  confirm({
    ...
    icon: <ExclamationCircleOutlined />,
    content:<>
    <strong>{stocksymbol}</strong> vs <strong> <Input onChange={compareStocks} /></strong>
  ...
  <TextArea placeholder="textarea with clear icon" allowClear onChange={onBange} />
    ...
    onOk() {
      ...
      .then((data) => console.log('Our data', localStorage.setItem('stockID', data[0].name), localStorage.setItem('stockname', data[0].symbol)))
      console.log("OK");
    ...
    console.log("Error getting document:", error);
      ...
      <div className="row">
        ...
        <div className="card">
          ...
          <div className="card">
            <Button type="primary">Compare onClick={testCompare}</Button>
          </div>
        </div>
