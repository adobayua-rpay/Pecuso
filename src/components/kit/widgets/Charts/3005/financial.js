...
export default function RecipeReviewCard() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl)
      .then((res) => res.json())
      .then((repos) => {
        setAppState({ loading: false, swot: repos.symbol, swock:repos.financials[0].grossProfit, COR:repos.financials[0].costofrevenue, operatingrevenue:repos.financials[0].operatingRevenue, totalrevenue:repos.financials[0].totalrevenue, operatingincome:repos.financials[0].operatingIncome, netincome:repos.financials[0].netIncome, rand:repos.financials[0].researchAndDevelopment,
        operatingexpense:repos.financials[0].operatingExpense, currentassets:repos.financials[0].currentAssets, totalassets:repos.financials[0].totalassets, totaliabilities:repos.financials[0].totaliabilities, currentcash:repos.financials[0].currentCash, currentdebt:repos.financials[0].currentDebt, shortermdebt:repos.financials[0].shortTermDebt, longtermdebt:repos.financials[0].longTermDebt, totalcash:repos.financials[0].totalCash, totalDebt:repos.financials[0].totalDebt, shareholderequity:repos.financials[0].shareholderEquity, cashchange:repos.financials[0].cashchange, cashFlow:repos.financials[0].cashFlow
        });
      }); 
  }, [setAppState]);

console.log("Swot" + "" + appState.swot)
const number = 9;
...
// class App extends React.Component {
  ...
  <Collapse defaultActiveKey={['1']} onChange={callback}>
    <Panel header="Report Date" key="1">
      <p>{text}</p>
      <strong>${appState.swock}</strong>

    </Panel>
    <Panel header="Gross Profit" key="2">
      <p>{grossprofit}</p>
      <strong>${appState.grossprofit}</strong>
    ...
    <Panel header="Cost of Revenue" key="3">
      <p>{costofrevenue}</p>
      <strong>{appState.costofrevenue}</strong>
    ...
    <Panel header="Operating Revenue" key="3" disabled>
      ...
      <strong>${appState.operatingrevenue}</strong>
      <p><b>How Operating Revenue Affects Stock Price</b></p>
      <p>{operatingRevenue2}</p>
      <Button type="link">Compare Operating Revenue</Button>

    </Panel>
    <Panel header="Total Revenue" key="4">
      <p>{totalrevenue}</p>
      <strong>${appState.totalrevenue}</strong>
    ...
    <Panel header="Operating Income" key="5">
      <p>{operatingincome}</p>
      <strong>${appState.operatingincome}</strong>
      <p><b>Operating Income Importance</b></p>
      <p>{operatingincome2}</p>
      <Button type="link">Compare Operating Income</Button>

    </Panel>

    <Panel header="Net Income" key="8">
      <p>{netincome}</p>
      <strong>${appState.netincome}</strong>
      <p><b>Why is netincome important to investors?</b></p>
      <p>{netincome2}</p>
      <Button type="link">Compare Net Income</Button>

    </Panel>
    <Panel header="Research and Development" key="9">
      <p>{rand}</p>
      <strong>${appState.rand}</strong>
      <p><b>Understanding Research and Development (R&D)</b></p>
      <p>{rand2}</p>
      <p><b>Is R&D correlated to stock price returns?</b></p>
      <p>{rand3}</p>
      <Button type="link">Compare Research and Development</Button>

    </Panel>
    <Panel header="Current Assets" key="10">
    <p>{currentassets}</p>
    <strong>${appState.currentassets}</strong>
      <p><b>Understanding Current Assets</b></p>
      <p>{currentassets2}</p>
      <Button type="link">Compare Current Assets</Button>

    </Panel>
    <Panel header="Total Assets" key="11">
      <p>{text}</p>
      <strong>${appState.totalassets}</strong>
      <Button type="link">Compare Total Assets</Button>

    </Panel>
    <Panel header="Total Liabilities" key="12">
      <p>{totaliability}</p>
      <strong>${appState.totaliabilities}</strong>
      <Button type="link">Compare Total Liabilities</Button>

    </Panel>
    <Panel header="Current Cash" key="13">
    <p>{cash}</p>
    <strong>${appState.currentcash}</strong>

      <p><b>What Cash & Cash Equivalents can tell you about a stock or a business</b></p>
      <p>{cash2}</p>
      <Button type="link">Compare Current Cash</Button>

    </Panel>
    <Panel header="Current Debt" key="14">
    <p>{debt}</p>
    <strong>${appState.currentdebt}</strong>

      <p><b>Good Debt vs Bad Debt</b></p>
      <p>{debt2}</p>
      <Button type="link">Compare Current Debt</Button>

    </Panel>
    <Panel header="Short Term Debt" key="15">
    <strong>${appState.shortermdebt}</strong>

      <p>{shortdebt}</p>
      
      <Button type="link">Compare Short Term Debt</Button>

    </Panel>
    <Panel header="Long Term Debt" key="16">
    <strong>${appState.longtermdebt}</strong>

      <p>{longdebt}</p>
      <Button type="link">Compare Long Term Debt</Button>

    </Panel>
    <Panel header="Total Cash" key="17">
    <strong>${appState.totalcash}</strong>

      <p>{text}</p>
      <Button type="link">Compare Total Cash</Button>

    </Panel>
    <Panel header="Total Debt" key="18">
    <strong>${appState.tot}</strong>

      <p>{text}</p>
      <Button type="link">Compare Total Debt</Button>
    ...
    <p>{sequity}</p>
      ...
      <p>{sequity3}</p>
      <Button type="link">Compare Shareholder Equity</Button>

    </Panel>
    <Panel header="Cash Change" key="20">
      <p>{text}</p>
      <Button type="link">Compare Cash Change</Button>

    </Panel>
    <Panel header="Cash Flow" key="21">
      <p>{text}</p>
      <Button type="link">Compare Cash Flow</Button>
